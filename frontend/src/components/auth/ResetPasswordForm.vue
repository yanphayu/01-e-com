  ...
  <form class="auth-card stack verify-email-form" @submit.prevent="handleSubmit">
    <div class="auth-head">
      ...
      <p>
        ...
        <strong>{{ email || t('auth.resetPasswordSubYour') }}</strong>.
      </p>
    </div>
    ...
    <button
      ...
      :disabled="loading || expired || codeLength !== 6"
    >
    ...
    <RouterLink to="/forgot-password" class="back-link">{{ t('auth.useDifferentEmail') }}</RouterLink>
  </form>
</template>
...
async function handleSubmit() {
  ...
  try {
    sessionStorage.setItem('resetOtp', code.value)
